...
static portTASK_FUNCTION(ADCTask,pvParameters)
{
    ...
    while(1)
    {
        ...
        configADC_DisparaSensorM();             //Dispara la conversion (por software)
        configADC_LeeSensorM(&Valor_SenM);    //Espera y lee muestras del ADC (BLOQUEANTE)

        ind = buscar_indice_medida(Lista_ADC, Valor_SenM, 0, 19);
        if(Lista_Distancia[ind]-1 < 4){                                           // Distancia menor de 4cm

            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1, 0);
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0);
        }
        else if(Lista_Distancia[ind] >= 4 && Lista_Distancia[ind] <= 14){       // Distancia entre 4 - 14 cm

            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1, 0);
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3);
        }
        else if(Lista_Distancia[ind] > 14 && Lista_Distancia[ind] <= 26){       // Distancia entre 14 - 26 cm

            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1, GPIO_PIN_1);
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0);
        }
        else if(Lista_Distancia[ind] > 26 && Lista_Distancia[ind] <= 40){       // Distancia entre 26 - 40 cm

            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1, GPIO_PIN_1);
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3);

       }
        else if(Lista_Distancia[ind] > 40){                                      // Distancia mas de 40 cm

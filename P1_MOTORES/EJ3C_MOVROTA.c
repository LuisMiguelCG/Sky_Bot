...
void GPIOFIntHandler(void)
{
    ...
    uint32_t ui32StatusPWM1, ui32StatusPWM2;

    // Boton Derecho: giro hacia la derecha
    if(((i32Status & RIGHT_BUTTON) == RIGHT_BUTTON)){

        //ESTADO = AVANCE;
        //TimerEnable(TIMER0_BASE, TIMER_A);

        ui32StatusPWM1 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_6);
        ui32StatusPWM2 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_7);

        if((ui32StatusPWM1 < ATRAS_DRC) && (ui32StatusPWM2 < ADELANTE_IZQ)){

            PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, ui32StatusPWM1 + CYCLE_INCREMENTS);
            PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7, ui32StatusPWM2 + CYCLE_INCREMENTS);
        }

    }
    // Boton Izquierdo: giro hacia la izquierda
    if(((i32Status & LEFT_BUTTON) == LEFT_BUTTON)){
        ...
        if((ui32StatusPWM1 > ADELANTE_DRC) && (ui32StatusPWM2 > ATRAS_IZQ)){
...
void Timer0IntHandler(void){
    ...
    uint32_t ui32StatusPWM1, ui32StatusPWM2 ;
    TimerIntClear(TIMER0_BASE, TIMER_TIMA_TIMEOUT); // Borra la interrupcion de Timer

    if(ESTADO == AVANCE) //**Giro derecha
    {
        ui32StatusPWM1 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_6);
        ui32StatusPWM2 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_7);
    ...
    if(ESTADO == RETROCESO)  //**Giro izquierda
    {
       ui32StatusPWM1 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_6);
        ui32StatusPWM2 = PWMPulseWidthGet(PWM1_BASE, PWM_OUT_7);
